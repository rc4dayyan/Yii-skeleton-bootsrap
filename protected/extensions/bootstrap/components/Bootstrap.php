...
class Bootstrap extends CApplicationComponent
{
	...
	public function registerJS($position = CClientScript::POS_HEAD)
	{
		/** @var CClientScript $cs */
		$cs = Yii::app()->getClientScript();
		$postend = CClientScript::POS_END;
		$cs->registerCoreScript('jquery');

		/** enable bootboxJS? */
		if($this->enableBootboxJS)
		{
			$cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap.bootbox.min.js', $postend);
		}
		$cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap' . (!YII_DEBUG ? '.min' : '.min') . '.js', $postend);
	}
